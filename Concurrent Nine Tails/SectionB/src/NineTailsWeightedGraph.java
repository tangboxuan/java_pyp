...
public class NineTailsWeightedGraph {
  ...
  private void constructGraph() {
    ...
    for (int i = 1; i <= NUM_CONFIGURATIONS; i++) {
      PriorityQueueInterface<WeightedEdge> pq = generateChildren(i);
      while (!pq.isEmpty()) {
        WeightedEdge e = pq.peek();
        configurations.get(e.child).add(e);
        pq.remove();
      }
    }
  }
   ...
   */
  private PriorityQueueInterface<WeightedEdge> generateChildren(int index) {
    PriorityQueueInterface<WeightedEdge> children = new PriorityQueue<WeightedEdge>();
   ...
   * <p>
   * flipResult includes the index of the generated configuration and the number of flips
   *
  ...
  public void printParentsTest(int index) {
    ...
    PriorityQueue<WeightedEdge> parents = configurations
        .get(index).clone();
  ...
  public void printShortestPath(int source) {
    ...
    System.out.println(
        "Shortest path from " + source + " to target configuration (" + TERMINAL_CONFIGURATION_INDEX
            + ") has cost of " + mst.costs[source] + ":");
    System.out.println("------------------------------");
  ...
  private void constructMinimumSpanningTree() {
    ...
    visited.add(visited.size() + 1, TERMINAL_CONFIGURATION_INDEX);
    costs[TERMINAL_CONFIGURATION_INDEX] = 0;
    PriorityQueueInterface<WeightedEdge> terminalPq = confCopy.get(TERMINAL_CONFIGURATION_INDEX);
    while (!terminalPq.isEmpty()) {
      WeightedEdge e = terminalPq.peek();
      nextMoves[e.parent] = TERMINAL_CONFIGURATION_INDEX;
      costs[e.parent] = e.weight;
      terminalPq.remove();
    }
    ...
    while (visited.size() < NUM_CONFIGURATIONS) {
      ...
      for (int i = 1; i <= NUM_CONFIGURATIONS; i++) {
        if (costs[i] < minValue && !visited.contains(i)) {
          minIndex = i;
          minValue = costs[i];
        }
      }

      visited.add(visited.size() + 1, minIndex);
      PriorityQueueInterface<WeightedEdge> pq = confCopy.get(minIndex);
      while (!pq.isEmpty()) {
        WeightedEdge e = pq.peek();
        if (e.weight + costs[minIndex] < costs[e.parent]) {
          nextMoves[e.parent] = minIndex;
          costs[e.parent] = e.weight + costs[minIndex];
        }
        pq.remove();
      }
    }
